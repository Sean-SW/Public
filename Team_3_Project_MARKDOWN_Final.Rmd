...
author: "Hanyu Chen,Lin Song,Rohan Gupta,Deniz Ozakyol,Yesol Lee, Shi Wang"
date: "10/7/2021"
...
##Loading the Dataset##
```{r}
diamond=fread("diamonds_dataset.csv")
```
...
#Training the Model
x.train_R <- model.matrix(f1_R, train_index)[, -1]
y.train_R <- train_index$logprice
x.test_R <- model.matrix(f1_R, test_index)[, -1]
y.test_R <- test_index$logprice
...
#Training the model
x.train_BF <- model.matrix(f1_BF, train_index)[, -1]
y.train_BF <- train_index$logprice
x.test_BF <- model.matrix(f1_BF, test_index)[, -1]
y.test_BF <- test_index$logprice


fit.btree <- gbm(f1_BF,train_index,
...
yhat.btree <- predict(fit.btree, train_index, n.trees = 100) 
mse.btree <- mean((yhat.btree - y.train_BF) ^ 2) 
print(mse.btree)

yhat.btree.test <- predict(fit.btree, test_index, n.trees = 100)
...
```{r}
A <- data.table(
  carat = x.test_RT$carat,
   logprice = x.test_RT$logprice,
  dataset = "Actual"
)
P <- rbind(A, data.table(
  carat = x.test_RT$carat,
  logprice = yhat.rndfor.test,
  dataset = "Predicted"))

ggplot(P, aes(carat, logprice, color=dataset)) + geom_smooth()
 # Checking Points where dimaond is overpriced and where it is underpriced against the most important variable
```